...
const ProfitAndLoss = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (submitForm) {
            setError('')
            targetId === 'monthlyPnlReport' ? setMonthlyLoading(true) : setDailyLoading(true)
            setAlertPopup('')
            ...
            // appending payReport type 
            targetId === 'monthlyPnlReport' ? formData.append('payReportType', 'monthly') : formData.append('payReportType', 'daily')
            try {
                    // formData.append('payReport', 'daily')
                    console.log('api call', formData)
                    await fetch('http://206.72.206.254:8000/api/profitLoss/upload', {
                        headers: {
                        ...
                        data.then((result) => {
                            ...
                            targetId === 'monthlyPnlReport' ? setMonthlyLoading(false) : setDailyLoading(false)
        ...
        <>
            <div className='mainDiv'>
                ...
                <form onSubmit={handleSubmit} className='formContainer'>
                    ...
                    {
              isFileUpdated ?
                <a href={`http://206.72.206.254:8000/api/profitLoss/download/xlsxFile=${fileName}`}>
                  <button className='uploadbutton' type="button">Download P&L Report</button>
                </a> :
                <div>
                    <button className='uploadbutton' id='dailyPnlReport' onClick={handleSubmit} type="submit">{dailyLoading ? <Spinner /> : 'Generate P&L Report'}</button>
                    <button className='uploadbutton' id='monthlyPnlReport' onClick={handleSubmit} type="submit">{monthlyLoading ? <Spinner /> : 'Generate Monthly P&L Report'}</button>
                </div>
                }
